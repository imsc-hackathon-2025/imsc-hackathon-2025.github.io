...
class LeaderboardManager {
    constructor(track) {
        ...
        this.sheetdb = new SheetDBIntegration('https://sheetdb.io/api/v1/your_sheet_id');
        
        // Method 2: Direct CSV (Simple)
        this.googleCSV = new GoogleSheetCSV('1BxiMVs0XRA5nFMdKvBdBZjgmUUqptlbs74OgvE2upms', 0);
        
        // Method 3: Apps Script Web App
        this.appsScript = new GoogleAppsScript('https://script.google.com/macros/s/your_script_id/exec');
    ...
    setupEventListeners() {
        ...
        filterButtons.forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                this.currentFilter = e.target.textContent.toLowerCase().replace(' ', '');
                this.applyFilters();
            });
        });
    ...
    normalizeGoogleSheetsData(rawData) {
        return rawData
            ...
            .map((row, index) => ({
                ...
                lastSubmission: row.timestamp || row.last_update || new Date().toISOString(),
                rank: index + 1
            }))
    ...
    getMockData() {
        ...
        const mockData = [
            { name: "Team Alpha", school: "Central High", score: 95.2, accuracy: 94.8, submissions: 5, lastSubmission: "2025-01-15T14:30:00Z" },
            { name: "Code Warriors", school: "Tech Academy", score: 92.7, accuracy: 91.5, submissions: 3, lastSubmission: "2025-01-15T13:45:00Z" },
            { name: "Data Miners", school: "Science High", score: 89.3, accuracy: 88.7, submissions: 4, lastSubmission: "2025-01-15T15:20:00Z" },
            { name: "Neural Network", school: "Central High", score: 87.1, accuracy: 85.9, submissions: 6, lastSubmission: "2025-01-15T12:15:00Z" },
            { name: "Binary Beasts", school: "Innovation School", score: 84.6, accuracy: 83.2, submissions: 2, lastSubmission: "2025-01-15T16:00:00Z" }
        ];
    ...
    renderTeamRow(team) {
        ...
        return `
            <div class="table-row ${team.rank <= 3 ? 'top-rank' : ''}" data-team-id="${team.id}">
                <div class="rank-col">
    ...
    updateStats(data) {
        const stats = {
            ...
            averageScore: (data.reduce((sum, team) => sum + team.score, 0) / data.length).toFixed(1)
        };
...
class GoogleSheetCSV {
    ...
    async fetchViaProxy() {
        ...
        const proxyUrl = `https://api.allorigins.win/get?url=${encodeURIComponent(this.csvUrl)}`;
        try {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const track = document.body.dataset.track || 
                  window.location.pathname.includes('high-school') ? 'high-school' :
                  window.location.pathname.includes('undergraduate') ? 'undergraduate' :
                  window.location.pathname.includes('graduate') ? 'graduate-plus' : 'unknown';
